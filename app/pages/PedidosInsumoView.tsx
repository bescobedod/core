    ...
    <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4" onClick={onClose}>
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-2xl max-h-[85vh] flex flex-col" onClick={e => e.stopPropagation()}>
        <div className="px-6 py-4 border-b border-gray-100 flex items-center justify-between shrink-0">
          <div>
            <h3 className="text-base font-semibold text-gray-900">Comparativo de stock — Insumos</h3>
...
export function PedidosInsumosView() {
  ...
  const [candado, setCandado] = useState<CandadoRutaInsumos | null>(null);

  const [rutasDisponibles, setRutasDisponibles] = useState<RutaInsumos[]>([]);
  ...
  // Al montar: revisa si ya hay un candado global activo, y siempre carga
  // el listado de rutas (haga falta o no de inmediato)
  useEffect(() => {
    (async () => {
      ...
      try {
        const candadoExistente = await getCandadoActivo();
        const rutas = await getRutasInsumos();
        ...
        setErrorInicial(err instanceof Error ? err.message : "Error al cargar la vista");
      } finally {
  ...
  const handleBuscarPreview = async () => {
    ...
    try {
      const data = await getPedidosPos("INSUMOS", {
  ...
  const handleLiberar = async () => {
    if (!candado) return;
    if (!window.confirm("¿Liberar esta ruta? Podrás retomarla después, pero otra persona podría tomar otra ruta de insumos mientras tanto.")) return;
  ...
  const handleCalcularStock = async () => {
    ...
    try {
      const data = await getComparativoStockInsumos(candado.fecha, candado.ruta_id);
  ...
  const handleGuardarAsignacionFifo = async () => {
    ...
    try {
      ...
      const data = await getPedidosPos("INSUMOS", { ruta_id: candado.ruta_id, fecha_requerida: candado.fecha });
      setPedidoRuta(data.rutas[0] || null);
    } catch (err) {
  ...
  const handleEnviarTransferencia = async () => {
    if (!candado) return;
    if (!window.confirm("¿Enviar la transferencia a SAP? Esto mueve el inventario a la bodega destino y libera la ruta.")) return;

    setEnviandoSap(true);
    setErrorEnviarSap(null);

    try {
      const resultado = await enviarTransferenciaInsumos(candado.ruta_id, candado.fecha);
  ...
  if (errorInicial) {
    return (
      <div className="max-w-lg mx-auto mt-10 bg-red-50 border border-red-200 rounded-xl px-4 py-4 flex items-start gap-2.5">
        ...
        <div>
          <p className="text-sm text-red-700 font-medium">No se pudo cargar la vista</p>
          <p className="text-xs text-red-600 mt-1">{errorInicial}</p>
        </div>
      </div>
    );
  }
      ...
      <div className="mb-6 bg-gradient-to-r from-[#2183AE] to-[#1a6a8f] rounded-2xl shadow-lg p-6 text-white">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-sm text-white/90 mt-0.5">Verifica los pedidos de insumos por ruta</p>
          </div>
        </div>
      </div>
        ...
        <div className="bg-white rounded-xl shadow-md border border-gray-200 p-5">
          ...
          {busquedaRealizada && (!previewRuta || previewRuta.tiendas.length === 0) && (
            <div className="flex flex-col items-center justify-center py-8 text-center border border-gray-100 rounded-xl mb-4">
              ...
              <p className="text-sm text-gray-400">No hay pedidos de insumos para esta ruta en esta fecha.</p>
            </div>
          )}
          ...
          {rutasDisponibles.length === 0 && (
            <p className="text-xs text-gray-400 mt-3">No hay rutas de insumos configuradas todavía — pídele a un administrador que las cree en el maestro de rutas.</p>
          )}
        </div>
      ) : (
        // ------------------------------------------------------------
