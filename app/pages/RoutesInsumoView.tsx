...
import { Package2, Plus, Store, Search, Loader2, AlertCircle, MapPin, Trash2, Pencil, Check } from "lucide-react";
import { Button } from "../ui/button";
import { Label } from "../ui/label";
import { Input } from "../ui/input";
import { getRutasInsumos, crearRutaInsumos, actualizarRutaInsumos, buscarTiendasPdv, getTiendasDeRuta, asignarTiendaRuta, quitarTiendaDeRuta } from "../api/RutaInsumoApi";
import { RutaInsumos, TiendaPdvBusqueda, TiendaRutaInsumos } from "../types/RutaInsumoModel";

export function RoutesInsumoView() {
  const [rutas, setRutas] = useState<RutaInsumos[]>([]);
  const [loadingRutas, setLoadingRutas] = useState(false);
  const [errorRutas, setErrorRutas] = useState<string | null>(null);

  const [rutaSeleccionada, setRutaSeleccionada] = useState<RutaInsumos | null>(null);
  const [tiendasDeRuta, setTiendasDeRuta] = useState<TiendaRutaInsumos[]>([]);
  ...
  const cargarRutas = async () => {
    ...
    try {
      const data = await getRutasInsumos();
      setRutas(data);
    } catch (err) {
  ...
  const seleccionarRuta = async (ruta: RutaInsumos) => {
  ...
  const handleAbrirEdicion = (e: React.MouseEvent, ruta: RutaInsumos) => {
    e.stopPropagation();
  ...
  const handleGuardarEdicion = async (e: React.MouseEvent, rutaId: string) => {
    ...
    try {
      await actualizarRutaInsumos(rutaId, editNombreRuta.trim(), editWhsDestino.trim());
      setEditandoRutaId(null);
      await cargarRutas();
    ...
    <div className="py-4 sm:py-6 lg:py-2 px-2 sm:px-4">
      <div className="mb-6 bg-gradient-to-r from-[#2183AE] to-[#1a6a8f] rounded-2xl shadow-lg p-6 text-white">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h2 className="text-white text-lg font-semibold leading-tight">Maestro de Rutas — Insumos</h2>
            <p className="text-sm text-white/90 mt-0.5">Define las rutas y asigna las tiendas de cada una</p>
          </div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-5 gap-4">
        ...
        <div className="lg:col-span-2 bg-white rounded-xl shadow-sm border border-gray-200 p-4">
          ...
          {loadingRutas ? (
            ...
            <p className="text-xs text-gray-400 text-center py-6">No hay rutas de insumos todavía.</p>
          ) : (
